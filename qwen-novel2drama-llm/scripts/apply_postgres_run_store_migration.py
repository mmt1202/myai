...
from agent.postgres_run_store import DEFAULT_POSTGRES_RUN_STORE_DSN_ENV, PostgresConnectionProfile, PostgresRunStore, load_schema_sql, split_sql_statements
...
def migration_plan(sql_path: Path) -> dict[str, Any]:
    sql = load_schema_sql(sql_path)
    statements = split_sql_statements(sql)
    return {"sql_path": str(sql_path), "statement_count": len(statements), "statements": [statement.splitlines()[0].strip() for statement in statements]}
...
def main() -> int:
    ...
    if args.dry_run:
        ...
        applied = store.init_db(sql_path=sql_path)
        store.close()
        result = {"status": "applied", "dsn": redact_configured_secret(dsn), "dsn_env": args.dsn_env, "connection_profile": profile.__dict__, **plan, "applied_statement_count": applied["statement_count"]}
    if args.json:
