...
- `agent/runtime.py`：通用 Agent run/session/step 状态机与审批门运行时。
...
P1_router_token_cost_usage_memory_rules_agent_runtime_provider_adapter_skills_mcp_adapter_api_server_implemented_v1 = true
...
P0 已经把核心研究结果转成第一版工程契约；P1 已经开始实现 router、token、cost、usage ledger、memory store、rule engine、agent runtime、provider adapter、skills registry、MCP adapter 和 API server integration。下一步应继续实现 Agent provider execution、Agent skill loop 和静态 OpenAPI 同步。

## 当前最重要的未完成项

### P1：底座核心服务剩余项

1. 将 `agent/runtime.py` 接入 provider factory，执行真实 provider call。
2. 将 Skills registry 接入 Agent tool loop。
3. 同步 `openapi/foundation_api.openapi.yaml` 中的 helper endpoints。
4. 增加 auth/API key/workspace scope。

### P2：Provider 接入与生产化

1. 本地 provider adapter。
...
- `agent/runtime.py` 已开始把 router 与 rules 串成通用 Agent run 状态机。
...
1. P1：继续实现 Agent provider execution、Agent skill loop、OpenAPI static contract sync、auth/API key scopes。
...
- 不能把“P1 router/token/cost/ledger/memory/rules/agent runtime/provider adapter/skills/MCP adapter/API server 完成”说成“所有核心服务完成”。
