...
- `openapi/foundation_api.openapi.yaml`：Foundation API OpenAPI 规格，已同步当前运行时 `/v1/*` endpoints、API key security scheme、ProviderStreamEvent、ProviderToolCall、Agent lifecycle schema、Agent run store selection、Agent event schema、model tool loop、stream provider tool bridge、incremental stream tool execution、same-stream continuation fallback events 和 workspace quota 字段。
...
- `agent/lifecycle.py`：Agent status、cancel、retry、resume 生命周期控制 CLI 与服务函数，已支持 `file`/`sqlite` run store 选择。
