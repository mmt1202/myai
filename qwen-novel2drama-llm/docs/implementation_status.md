...
- Run store、SQLite run store、Postgres run store contract/migration runner/history、worker dispatcher、provider continuation/session adapters、provider smoke config、Agent events、Agent runtime、Agent lifecycle、API server、auth、audit/rate limit、workspace quota、quota store、Postgres quota、skills、MCP 等测试已补充。
...
1. API middleware-level quota checks for selected scopes。
2. Production deployment profile / health checks / observability dashboards。
3. Provider-specific production session lifecycle hardening。
4. P2 多 provider、多模态、MCP SDK 兼容层、评测、观测、审计、部署。
5. P3 AI 短剧/漫剧专项能力。

## 后续开发顺序

1. P1 hardening：API middleware quota、deployment profile、pool health checks、queue observability、provider session lifecycle hardening。
...
- OpenAI Responses continuation 完成，不等于 WebSocket Realtime 连接生命周期由本项目全托管。
- OpenAI Realtime session adapter 完成，需要调用方传入已打开的 realtime session；它不等于浏览器/WebRTC/SIP 接入完成。
- Worker queue dispatcher 完成，不等于外部消息队列、跨区域分布式调度器或自动 worker pool 完成。
- Postgres migration history 完成，不等于完整回滚系统或数据库运维治理完成。
