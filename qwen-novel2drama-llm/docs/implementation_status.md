...
P0 已经把核心研究结果转成第一版工程契约；P1 已经开始实现 router、token、cost、usage ledger、memory store、rule engine、agent runtime、provider adapter、skills registry、MCP adapter、API server integration、Agent provider execution 和 Agent skill loop。下一步应继续实现静态 OpenAPI 同步、auth/API key/workspace scope 和本地 provider adapter。

## 当前最重要的未完成项

### P1：底座核心服务剩余项

1. 同步 `openapi/foundation_api.openapi.yaml` 中的 helper endpoints。
2. 增加 auth/API key/workspace scope。
3. 增加本地 provider adapter，把已有本地模型运行时接入 provider factory。
4. 将 request-driven skill loop 升级为 model-decided multi-turn tool loop。
...
1. P1：继续实现 OpenAPI static contract sync、auth/API key scopes、本地 provider adapter、model-decided tool loop。
