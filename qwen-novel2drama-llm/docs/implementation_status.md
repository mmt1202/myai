...
- `tests/test_provider_adapter_contract.py`：Provider adapter 与 ProviderStreamEvent 合同测试。
...
P0/P1 已经把核心研究结果转成第一版工程契约和一批可运行服务；OpenAPI 已同步当前 FastAPI runtime，API server 已接入轻量 API key/workspace scope、auth audit、文件状态限流、Agent JSON/SSE 事件接口和 provider SSE streaming，本地模型已接入统一 provider factory，并具备进程内缓存、并发保护和流式输出入口，Agent 已具备同步 model-decided tool loop v1。下一步应继续实现 native OpenAI-compatible streaming、CI contract check 和 workspace-level budget/quota。

## 当前最重要的未完成项

### P1：底座核心服务剩余项

1. 增加 native OpenAI-compatible streaming。
...
1. P1：继续实现 native OpenAI-compatible streaming、CI contract check、workspace-level budget/quota、Agent resume/cancel/retry。
