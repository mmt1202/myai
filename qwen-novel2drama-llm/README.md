...
你后续要做的不是单一短剧模型，而是一个可支撑桌面编程助手、Skill、插件、MCP、内容生产和 AI 短剧/漫剧平台的本地通用大模型基座。

本项目现在拆成两层：

1. `foundation_base`：通用助手、编程助手、Agent/工具规划、插件/MCP 意识、结构化输出。
2. `novel2drama`：AI 短剧/AI 漫剧垂直内容生产能力。

通用基座配置：

```bash
bash scripts/train_lora.sh configs/qwen2_5_3b_foundation_lora.yaml
bash scripts/train_lora.sh configs/qwen2_5_7b_foundation_qlora.yaml
```

详细策略见 `docs/foundation_model_strategy.md`。
