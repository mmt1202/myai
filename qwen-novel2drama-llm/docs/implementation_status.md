...
P1_router_token_cost_usage_memory_rules_agent_runtime_implemented_v1 = true
...
P0 已经把核心研究结果转成第一版工程契约；P1 已经开始实现 router、token、cost、usage ledger、memory store、rule engine 和 agent runtime。下一步应继续实现 provider adapter base、skills registry、MCP adapter 和 API server 集成。
...
3. `mcp/adapter.py`
4. 把 `inference/api_server.py` 接入新的 router/token/cost/memory/rules/agent/envelope。
...
1. P1：继续实现 provider adapter base、skills registry、MCP adapter、API server route integration。
...
- 不能把“P1 router/token/cost/ledger/memory/rules/agent runtime 完成”说成“所有核心服务完成”。
