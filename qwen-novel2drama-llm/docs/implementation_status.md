...
P0/P1 已经把核心研究结果转成第一版工程契约和一批可运行服务；OpenAPI 已同步当前 FastAPI runtime，API server 已接入轻量 API key/workspace scope，本地模型也已接入统一 provider factory，Agent 已具备同步 model-decided tool loop v1，并已加入 OpenAPI/runtime 合同检查脚本。下一步应继续实现 auth audit/rate limiting、本地 provider 并发/流式和 streaming run events。

## 当前最重要的未完成项

### P1：底座核心服务剩余项

1. 增加 auth audit log、rate limit、workspace-level quota/budget。
2. 增加本地 provider 并发保护、缓存控制和流式输出。
3. 增加 Agent streaming run events 与 resume/cancel/retry。
4. 把 OpenAPI 合同检查加入 CI。
...
- `scripts/check_openapi_contract.py` 已开始检查 runtime `/v1/*` routes 与 OpenAPI paths 的一致性。

## 后续开发顺序

1. P1：继续实现 auth audit/rate limiting、本地 provider 并发/流式、Agent streaming events、CI contract check。
...
- 不能把“P1 router/token/cost/ledger/memory/rules/auth/agent runtime/provider adapter/skills/MCP adapter/API server/Agent provider execution/Agent skill loop/local provider/model tool loop/OpenAPI check 完成”说成“所有核心服务完成”。
