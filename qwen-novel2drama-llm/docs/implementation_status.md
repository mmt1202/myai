...
- `agent/run_store.py`：Agent run store 抽象、`FileRunStore`、`build_run_store()` 和 artifact index 接口，支持 file/sqlite 选择。
- `agent/sqlite_run_store.py`：本地 SQLite Agent run store v1，使用标准库 `sqlite3` 持久化 runs、requests、reports、events、cancel markers 和 artifact index。
- `agent/runtime.py`：通用 Agent run/session/step 状态机、事件流、审批门、skill loop、provider execution、stream provider tool bridge、incremental stream tool execution、provider usage reconciliation、workspace quota preflight/actual recording、取消 checkpoint 与 model tool loop 运行时；已支持可选 run store 写入 request/report/artifact index/event index，并可读取 store cancel marker。
...
- `inference/api_server.py`：统一 Foundation `/v1/*` API server 集成，已接入 auth、audit、rate limit middleware、Agent events JSON/SSE endpoint、Agent lifecycle status/cancel/retry/resume endpoints、file/sqlite Agent run store 选择和 `/v1/chat` provider SSE streaming。

### 测试与文档

- `tests/test_run_store.py`：Agent run store 抽象、`FileRunStore` 与 run store factory 测试。
- `tests/test_sqlite_run_store.py`：SQLiteRunStore schema、request/report、event、cancel marker、status 与 artifact index 测试。
...
- `tests/test_api_server_foundation.py`：Foundation API server、provider streaming、Agent lifecycle API 与 SQLite run store 选择测试。
- `docs/p1_agent_run_store.md`：Agent run store 抽象、SQLite store 与 file/sqlite 选择说明。
- `docs/p1_agent_runtime_run_store_writes.md`：Agent runtime run store writes 说明。
- `docs/p1_api_server_integration.md`：API server 集成、run store 选择与 Agent lifecycle API 说明。
...
P0/P1 已经把核心研究结果转成第一版工程契约和一批可运行服务；OpenAPI 已同步当前 FastAPI runtime，API server 已接入轻量 API key/workspace scope、auth audit、文件状态限流、Agent JSON/SSE 事件接口、Agent lifecycle status/cancel/retry/resume API、file/sqlite Agent run store 选择和 provider SSE streaming；Agent runtime 已能在传入 run store 时写入 request/report/artifact index/event index，并优先识别 store cancel marker；Agent lifecycle 已可通过 `FileRunStore` 或 `SQLiteRunStore` 读写 request/report/cancel marker；本地模型已接入统一 provider factory，并具备进程内缓存、并发保护和流式输出入口；OpenAI-compatible provider 已支持原生远程 SSE 文本流解析和 tool-call 分片重组。下一步应继续实现 run listing/query、DB-backed Agent events、Postgres run store、distributed quota backend 或 provider-native bidirectional adapter。

## 当前最重要的未完成项

### P1：底座核心服务剩余项

1. 增加 run listing/query API。
2. 增加 DB-backed Agent events，并让 Agent SSE 可从 DB event cursor 读取。
3. 增加 Postgres run store 实现。
4. 增加 provider-native bidirectional continuation adapter。
5. 增加分布式 quota/rate limit backend。
6. 增加 secret-gated real provider smoke tests。
...
1. P1：继续实现 run listing/query、DB-backed Agent events、Postgres run store、provider-native bidirectional continuation adapter、distributed quota backend、secret-gated provider smoke profiles。
2. P2：接入多 provider、多模态生成/理解、MCP SDK 兼容层、评测、观测、审计、部署。
3. P3：加强 AI 短剧/漫剧专项模型能力，包括故事理解、集数规划、角色一致性、分镜规划、视觉提示词生成、短剧质检评测。

## 禁止误判

- 不能把“Agent runtime run store writes 完成”说成“DB-backed events、run listing/query、Postgres run store 或分布式任务队列完成”。
- 不能把“Agent run store selection 完成”说成“runtime artifact 已全量 DB 写入、Postgres run store、分布式任务队列或事务型调度完成”。
