...
- `agent/runtime.py`：通用 Agent run/session/step 状态机、事件流、审批门、skill loop、provider execution、stream provider tool bridge、incremental stream tool execution 与 model tool loop 运行时。
...
- `tests/test_agent_runtime.py`：Agent runtime、stream provider tool bridge 与 incremental stream tool execution 测试。
...
1. P1：继续实现 provider usage reconciliation、CI contract check、workspace-level budget/quota、Agent resume/cancel/retry。
...
- 不能把“P1 router/token/cost/ledger/memory/rules/auth/audit/rate limit/agent runtime/provider adapter/skills/MCP adapter/API server/Agent provider execution/Agent skill loop/local provider/model tool loop/OpenAPI check/streaming 完成”说成“所有核心服务完成”。
