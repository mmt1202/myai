...
- `openapi/foundation_api.openapi.yaml`：Foundation API OpenAPI 规格，已同步当前运行时 `/v1/*` endpoints、API key security scheme、Agent event schema 和 model tool loop 字段。
...
- `providers/local_text.py`：本地 transformers 文本 provider adapter，已接入现有 `inference/model_utils.py`，并具备进程内缓存、加载锁、生成串行保护、cache stats 和 clear cache。
...
- `inference/api_server.py`：统一 Foundation `/v1/*` API server 集成，并已接入 auth、audit、rate limit middleware、Agent events JSON/SSE endpoint。
...
- `tests/test_local_text_provider.py`：本地 provider 缓存与执行测试。
...
P0/P1 已经把核心研究结果转成第一版工程契约和一批可运行服务；OpenAPI 已同步当前 FastAPI runtime，API server 已接入轻量 API key/workspace scope、auth audit、文件状态限流和 Agent JSON/SSE 事件接口，本地模型已接入统一 provider factory，并具备进程内缓存与并发保护，Agent 已具备同步 model-decided tool loop v1。下一步应继续实现本地 provider 流式输出、CI contract check 和 workspace-level budget/quota。

## 当前最重要的未完成项

### P1：底座核心服务剩余项

1. 增加本地 provider 流式输出入口。
...
1. P1：继续实现本地 provider 流式、CI contract check、workspace-level budget/quota、Agent resume/cancel/retry。
...
- 不能把“本地 provider 缓存/并发保护完成”说成“本地 provider 已支持流式、GPU 调度或多进程隔离”。
...
- 不能把“P1 router/token/cost/ledger/memory/rules/auth/audit/rate limit/agent runtime/provider adapter/skills/MCP adapter/API server/Agent provider execution/Agent skill loop/local provider/model tool loop/OpenAPI check 完成”说成“所有核心服务完成”。
