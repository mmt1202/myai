...
- `providers/realtime_base.py`：provider-native continuation adapter boundary 和 test-double adapter。
- `providers/openai_compatible.py`：支持显式 provider-native test continuation protocol 路由。
- `agent/tool_loop.py`：Agent stream bridge 已记录 provider-native continuation event 统计。
- `scripts/provider_smoke_test.py` 与 `.github/workflows/foundation-provider-smoke.yml`：provider smoke config/dry-run profile。

### 测试与文档

- Run store、SQLite run store、Postgres run store contract/migration runner、provider continuation、provider smoke config、Agent events、Agent runtime、Agent lifecycle、API server、auth、audit/rate limit、workspace quota、quota store、Postgres quota、skills、MCP 等测试已补充。
...
1. 完整 worker queue / dispatcher。
2. Postgres schema version tracking / migration history table。
3. Real provider-native session adapters for OpenAI Realtime/Responses。

## 后续开发顺序

1. P1：worker queue/dispatcher、Postgres migration history、real provider-native session adapters。
2. P2：多 provider、多模态生成/理解、MCP SDK 兼容层、评测、观测、审计、部署。
3. P3：AI 短剧/漫剧专项能力。

## 禁止误判

- Provider smoke workflow/profile 完成，不等于默认 CI 会发起真实 provider 调用；真实调用仍由环境配置显式启用。
- Provider-native continuation adapter v1 完成，不等于真实 OpenAI Realtime/Responses 会话已经接入。
...
Explicit non-goals: this milestone does not deliver complete billing, globally strongly-consistent distributed rate limiting, or a production-grade billing system.
