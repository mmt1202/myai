...
- `openapi/foundation_api.openapi.yaml`：Foundation API OpenAPI 规格，已同步当前运行时 `/v1/*` endpoints、API key security scheme、ProviderStreamEvent、ProviderToolCall、Agent event schema、model tool loop、stream provider tool bridge 和 incremental stream tool execution 字段。
- `scripts/check_openapi_contract.py`：OpenAPI/runtime 合同一致性检查脚本。
- `.github/workflows/foundation-contract-check.yml`：OpenAPI/runtime 合同检查与轻量核心测试 CI workflow。
...
- `agent/runtime.py`：通用 Agent run/session/step 状态机、事件流、审批门、skill loop、provider execution、stream provider tool bridge、incremental stream tool execution、provider usage reconciliation 与 model tool loop 运行时。
- `agent/tool_loop.py`：模型返回 tool calls 后的工具解析、skill 执行、tool_result 回填、多轮 provider 调用、provider stream chunks 到 provider response 的桥接，以及完整 partial tool call 的增量执行。
- `providers/base.py`：Provider adapter 基础合同，已包含 ProviderStreamEvent 标准 chunk 结构和 fallback stream_generate。
...
- `providers/factory.py`：按模型实例构造 provider 的工厂，已支持 OpenAI-compatible、local transformers 与 `stream_generate_with_registry`。
- `skills/registry.py`：Foundation Skills 注册表和调用入口。
- `mcp/adapter.py`：Foundation Skills 到 MCP-style tools/resources/prompts 的适配器。
- `inference/api_server.py`：统一 Foundation `/v1/*` API server 集成，并已接入 auth、audit、rate limit middleware、Agent events JSON/SSE endpoint 和 `/v1/chat` provider SSE streaming。
...
- `tests/test_agent_runtime.py`：Agent runtime、stream provider tool bridge、incremental stream tool execution 与 provider usage reconciliation 测试。
...
- `tests/test_api_server_foundation.py`：Foundation API server 与 provider streaming 测试。
...
- `docs/p1_usage_reconciliation.md`：Provider usage reconciliation 说明。
...
P0/P1 已经把核心研究结果转成第一版工程契约和一批可运行服务；OpenAPI 已同步当前 FastAPI runtime，API server 已接入轻量 API key/workspace scope、auth audit、文件状态限流、Agent JSON/SSE 事件接口和 provider SSE streaming，本地模型已接入统一 provider factory，并具备进程内缓存、并发保护和流式输出入口，OpenAI-compatible provider 已支持原生远程 SSE 文本流解析和 tool-call 分片重组，Agent 已能把 provider stream 完成后的 tool calls 桥接到同步 model tool loop，并能在 streamed partial tool call 的 name 与 JSON arguments 完整后提前执行 skill；provider usage reconciliation 已能对比路由预估 usage/cost 与 provider 实际 usage/cost，并把对账结果写回 Agent artifact 与 usage ledger；GitHub Actions 已接入 OpenAPI/runtime 合同检查和轻量核心测试。下一步应继续实现 workspace-level budget/quota。

## 当前最重要的未完成项

### P1：底座核心服务剩余项

1. 增加 workspace-level quota/budget 与分布式限流 backend。
2. 增加 Agent resume/cancel/retry。
3. 增加 same-stream tool-result injection 或 provider-supported bidirectional tool continuation。
4. 增加 multi-round model tool-loop usage aggregation。
5. 增加 heavyweight provider/model CI dependency profiles。
...
1. P1：继续实现 workspace-level budget/quota、Agent resume/cancel/retry、multi-round model tool-loop usage aggregation、heavyweight provider/model CI dependency profiles。
...
- 不能把“P1 router/token/cost/ledger/memory/rules/auth/audit/rate limit/agent runtime/provider adapter/skills/MCP adapter/API server/Agent provider execution/Agent skill loop/local provider/model tool loop/OpenAPI check/streaming/reconciliation/CI check 完成”说成“所有核心服务完成”。
