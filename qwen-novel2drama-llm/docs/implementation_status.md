...
P0/P1 已经把核心研究结果转成第一版工程契约和一批可运行服务；OpenAPI 已同步当前 FastAPI runtime，API server 已接入轻量 API key/workspace scope、auth audit 和文件状态限流，本地模型也已接入统一 provider factory，Agent 已具备同步 model-decided tool loop v1。下一步应继续实现本地 provider 并发/缓存/流式、Agent streaming run events、CI contract check 和 workspace-level budget/quota。

## 当前最重要的未完成项

### P1：底座核心服务剩余项

1. 增加本地 provider 并发保护、缓存控制和流式输出。
...
1. P1：继续实现本地 provider 并发/流式、Agent streaming events、CI contract check、workspace-level budget/quota。
