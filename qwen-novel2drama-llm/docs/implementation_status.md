...
- `agent/runtime.py`：通用 Agent run/session/step 状态机、审批门与 provider execution 运行时。
...
P1_router_token_cost_usage_memory_rules_agent_runtime_provider_adapter_skills_mcp_adapter_api_server_agent_provider_execution_implemented_v1 = true
...
P0 已经把核心研究结果转成第一版工程契约；P1 已经开始实现 router、token、cost、usage ledger、memory store、rule engine、agent runtime、provider adapter、skills registry、MCP adapter、API server integration 和 Agent provider execution。下一步应继续实现 Agent skill loop、静态 OpenAPI 同步和 auth/API key/workspace scope。

## 当前最重要的未完成项

### P1：底座核心服务剩余项

1. 将 Skills registry 接入 Agent tool loop。
2. 同步 `openapi/foundation_api.openapi.yaml` 中的 helper endpoints。
3. 增加 auth/API key/workspace scope。
4. 增加本地 provider adapter，把已有本地模型运行时接入 provider factory。
...
- `agent/runtime.py` 已开始把 router、rules、provider factory 和 usage ledger 串成通用 Agent run 状态机。
- `providers/factory.py` 已开始按模型实例构造 provider。
- `skills/registry.py` 已开始把内部服务注册为可调用 Skills。
- `mcp/adapter.py` 已开始把 Skills 暴露为 MCP-style tools/resources/prompts。
- `inference/api_server.py` 已开始把底座能力暴露为 `/v1/*` HTTP API。

## 后续开发顺序

1. P1：继续实现 Agent skill loop、OpenAPI static contract sync、auth/API key scopes、本地 provider adapter。
...
- 不能把“P1 router/token/cost/ledger/memory/rules/agent runtime/provider adapter/skills/MCP adapter/API server/Agent provider execution 完成”说成“所有核心服务完成”。
- 不能把“provider adapter base 完成”说成“所有模型供应商都已经接入”。
- 不能把“Agent provider execution 完成”说成“Agent tool loop 已完成”。
