...
P1_router_token_cost_usage_memory_rules_agent_runtime_provider_adapter_implemented_v1 = true
...
P0 已经把核心研究结果转成第一版工程契约；P1 已经开始实现 router、token、cost、usage ledger、memory store、rule engine、agent runtime 和 provider adapter。下一步应继续实现 skills registry、MCP adapter 和 API server 集成。
...
2. `mcp/adapter.py`
3. 把 `inference/api_server.py` 接入新的 router/token/cost/memory/rules/agent/provider/envelope。
4. 将 `agent/runtime.py` 接入 provider factory，执行真实 provider call。
...
1. P1：继续实现 skills registry、MCP adapter、API server route integration、Agent provider execution。
...
- 不能把“P1 router/token/cost/ledger/memory/rules/agent runtime/provider adapter 完成”说成“所有核心服务完成”。
