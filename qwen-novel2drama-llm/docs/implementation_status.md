...
2. Postgres/distributed quota backend。
3. 完整 worker queue / dispatcher。
4. Postgres schema version tracking / migration history table。
5. Real provider-native session adapters for OpenAI Realtime/Responses。

## 后续开发顺序

1. P1：secret-gated provider smoke profiles、Postgres/distributed quota backend、worker queue/dispatcher、Postgres migration history、real provider-native session adapters。
