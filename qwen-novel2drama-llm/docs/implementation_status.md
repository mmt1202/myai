...
- `openapi/foundation_api.openapi.yaml`：Foundation API OpenAPI 规格，已同步当前运行时 `/v1/*` endpoints 与 API key security scheme。
...
- `agent/runtime.py`：通用 Agent run/session/step 状态机、审批门、skill loop 与 provider execution 运行时。
...
P0/P1 已经把核心研究结果转成第一版工程契约和一批可运行服务；OpenAPI 已同步当前 FastAPI runtime，API server 已接入轻量 API key/workspace scope。下一步应继续实现本地 provider adapter、model-decided multi-turn tool loop、OpenAPI lint/check tooling 和 auth audit/rate limiting。

## 当前最重要的未完成项

### P1：底座核心服务剩余项

1. 增加本地 provider adapter，把已有本地模型运行时接入 provider factory。
2. 将 request-driven skill loop 升级为 model-decided multi-turn tool loop。
3. 增加 OpenAPI lint/check tooling。
4. 增加 auth audit log、rate limit、workspace-level quota/budget。
...
- `agent/runtime.py` 已开始把 router、rules、skills、provider factory 和 usage ledger 串成通用 Agent run 状态机。
...
- `openapi/foundation_api.openapi.yaml` 已同步当前运行时 endpoints、Agent provider/skill loop 字段和 API key security scheme。

## 后续开发顺序

1. P1：继续实现本地 provider adapter、model-decided tool loop、OpenAPI lint/check tooling、auth audit/rate limiting。
...
- 不能把“P1 router/token/cost/ledger/memory/rules/auth/agent runtime/provider adapter/skills/MCP adapter/API server/Agent provider execution/Agent skill loop 完成”说成“所有核心服务完成”。
- 不能把“provider adapter base 完成”说成“所有模型供应商都已经接入”。
- 不能把“Agent skill loop 完成”说成“模型自主多轮 tool loop 已完成”。
