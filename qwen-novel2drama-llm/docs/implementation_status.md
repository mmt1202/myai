...
P1_router_token_cost_usage_implemented_v1 = true
...
P0 已经把核心研究结果转成第一版工程契约；P1 已经开始实现 router、token、cost 和 usage ledger。下一步应继续实现 memory、rules、agent runtime 和 provider adapter base。

## 当前最重要的未完成项

### P1：底座核心服务剩余项

1. `services/memory_store.py`
2. `services/rule_engine.py`
3. `agent/runtime.py`
4. `providers/base.py`
5. `skills/registry.py`
6. `mcp/adapter.py`
7. 把 `inference/api_server.py` 接入新的 router/token/cost/envelope。
...
1. P1：继续实现 memory、rules、agent runtime、provider adapter base、API server route integration。
...
- 不能把“P1 router/token/cost/ledger 完成”说成“所有核心服务完成”。
