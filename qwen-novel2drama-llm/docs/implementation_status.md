...
P1_router_token_cost_usage_memory_rules_implemented_v1 = true
...
P0 已经把核心研究结果转成第一版工程契约；P1 已经开始实现 router、token、cost、usage ledger、memory store 和 rule engine。下一步应继续实现 agent runtime 和 provider adapter base。
...
1. `agent/runtime.py`
2. `providers/base.py`
3. `skills/registry.py`
4. `mcp/adapter.py`
5. 把 `inference/api_server.py` 接入新的 router/token/cost/memory/rules/envelope。
...
1. P1：继续实现 agent runtime、provider adapter base、API server route integration。
...
- 不能把“P1 router/token/cost/ledger/memory/rules 完成”说成“所有核心服务完成”。
