...
- `agent/postgres_run_store.py`：支持 Postgres request/report/event/cancel/artifact/list/status/lease persistence。
- `migrations/postgres_run_store.sql`：Postgres run store schema v1。
- `requirements/postgres-run-store.txt`：Postgres run store optional dependency profile。
- API server 可通过 run store env 选择 file/sqlite/postgres。

### 测试与文档

- Run store、SQLite run store、Postgres run store contract、Agent events、Agent runtime、Agent lifecycle、API server、auth、audit/rate limit、workspace quota、quota store、skills、MCP 等测试已补充。
...
1. Postgres migration runner / connection pool profile。
...
1. P1：Postgres migration runner/connection pool、provider-native bidirectional continuation、secret-gated provider smoke profiles、Postgres/distributed quota backend、worker queue/dispatcher。
2. P2：多 provider、多模态生成/理解、MCP SDK 兼容层、评测、观测、审计、部署。
3. P3：AI 短剧/漫剧专项能力。

## 禁止误判

- PostgresRunStore persistence v1 完成，不等于 migration runner、连接池、运维部署或完整生产数据库治理完成。
