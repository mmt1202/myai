...
P1_router_token_cost_usage_memory_rules_agent_runtime_provider_adapter_skills_mcp_adapter_implemented_v1 = true
...
P0 已经把核心研究结果转成第一版工程契约；P1 已经开始实现 router、token、cost、usage ledger、memory store、rule engine、agent runtime、provider adapter、skills registry 和 MCP adapter。下一步应继续实现 API server 集成、Agent provider execution 和 Agent skill loop。

## 当前最重要的未完成项

### P1：底座核心服务剩余项

1. 把 `inference/api_server.py` 接入新的 router/token/cost/memory/rules/agent/provider/skills/mcp/envelope。
2. 将 `agent/runtime.py` 接入 provider factory，执行真实 provider call。
3. 将 Skills registry 接入 Agent tool loop。
4. 将 MCP adapter 接入 `/v1/mcp/tools`。
...
1. P1：继续实现 API server route integration、Agent provider execution、Agent skill loop。
...
- 不能把“P1 router/token/cost/ledger/memory/rules/agent runtime/provider adapter/skills/MCP adapter 完成”说成“所有核心服务完成”。
