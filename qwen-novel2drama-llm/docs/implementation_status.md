...
P0/P1 已经把核心研究结果转成第一版工程契约和一批可运行服务；OpenAPI 已同步当前 FastAPI runtime，API server 已接入轻量 API key/workspace scope，本地模型也已接入统一 provider factory。下一步应继续实现 model-decided multi-turn tool loop、OpenAPI lint/check tooling 和 auth audit/rate limiting。

## 当前最重要的未完成项

### P1：底座核心服务剩余项

1. 将 request-driven skill loop 升级为 model-decided multi-turn tool loop。
2. 增加 OpenAPI lint/check tooling。
3. 增加 auth audit log、rate limit、workspace-level quota/budget。
4. 增加本地 provider 并发保护、缓存控制和流式输出。
...
1. P1：继续实现 model-decided tool loop、OpenAPI lint/check tooling、auth audit/rate limiting、本地 provider 并发和流式。
...
- 不能把“P1 router/token/cost/ledger/memory/rules/auth/agent runtime/provider adapter/skills/MCP adapter/API server/Agent provider execution/Agent skill loop/local provider 完成”说成“所有核心服务完成”。
