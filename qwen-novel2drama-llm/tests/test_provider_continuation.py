...
from providers.realtime_base import TestDoubleContinuationAdapter, adapter_for_protocol
...
class ProviderContinuationTests(unittest.TestCase):
    ...
    def test_adapter_for_protocol_is_explicit(self) -> None:
        ...
        self.assertIsNone(adapter_for_protocol("openai_realtime"))
    ...
    def test_factory_routes_provider_native_continuation(self) -> None:
        events = list(
            continue_stream_with_tool_result_with_registry(
                ...
                self.supported_registry(),
                model_id="m1",
                base_url="http://example.test/v1",
                tool_call={"id": "call_1", "name": "foundation.token_count"},
                tool_result={"tool_call_id": "call_1", "status": "ok"},
            )
        )
