...
- `agent/lifecycle.py`：Agent 文件状态版 status、cancel、retry、resume 生命周期控制 CLI 与服务函数。
- `agent/tool_loop.py`：模型返回 tool calls 后的工具解析、skill 执行、tool_result 回填、多轮 provider 调用、provider stream chunks 到 provider response 的桥接、完整 partial tool call 的增量执行，以及多轮 provider usage/cost 聚合 artifact 写入。
...
- `docs/p1_agent_runtime.md`：Agent runtime 与 lifecycle 说明。
...
P0/P1 已经把核心研究结果转成第一版工程契约和一批可运行服务；OpenAPI 已同步当前 FastAPI runtime，API server 已接入轻量 API key/workspace scope、auth audit、文件状态限流、Agent JSON/SSE 事件接口和 provider SSE streaming，本地模型已接入统一 provider factory，并具备进程内缓存、并发保护和流式输出入口，OpenAI-compatible provider 已支持原生远程 SSE 文本流解析和 tool-call 分片重组，Agent 已能把 provider stream 完成后的 tool calls 桥接到同步 model tool loop，并能在 streamed partial tool call 的 name 与 JSON arguments 完整后提前执行 skill；provider usage reconciliation 已能对比路由预估 usage/cost 与 provider 实际 usage/cost，并把对账结果写回 Agent artifact 与 usage ledger；model tool loop 多轮 provider 调用 usage/cost 已能聚合成总账 artifact；workspace quota 已支持 Agent provider execution 的预执行配额检查与实际用量入账；Agent lifecycle 已支持文件状态版 status、cancel、retry、resume；GitHub Actions 已接入 OpenAPI/runtime 合同检查和轻量核心测试。下一步应继续实现 same-stream tool-result injection 或 distributed quota backend。

## 当前最重要的未完成项

### P1：底座核心服务剩余项

1. 增加 same-stream tool-result injection 或 provider-supported bidirectional tool continuation。
2. 增加分布式 quota/rate limit backend。
3. 增加 heavyweight provider/model CI dependency profiles。
4. 增加 lifecycle API endpoint 或数据库 run store。
5. 增加 provider-specific billing/invoice reconciliation。
...
1. P1：继续实现 same-stream tool-result injection、distributed quota backend、heavyweight provider/model CI dependency profiles、lifecycle API endpoint。
...
- 不能把“P1 router/token/cost/ledger/memory/rules/auth/audit/rate limit/workspace quota/agent lifecycle/agent runtime/provider adapter/skills/MCP adapter/API server/Agent provider execution/Agent skill loop/local provider/model tool loop/OpenAPI check/streaming/reconciliation/CI check 完成”说成“所有核心服务完成”。
