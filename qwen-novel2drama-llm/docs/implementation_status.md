...
- `agent/run_store.py`：Agent run store 抽象、`FileRunStore` 与 `build_run_store()`，支持 file/sqlite 选择。
...
- `agent/lifecycle.py`：Agent status、cancel、retry、resume 生命周期控制 CLI 与服务函数，已支持 `--run-store file|sqlite` 和 `--sqlite-path`。
...
1. P1：继续实现 Agent runtime artifact 写入迁移、run listing/query、DB-backed Agent events、Postgres run store、provider-native bidirectional continuation adapter、distributed quota backend、secret-gated provider smoke profiles。
