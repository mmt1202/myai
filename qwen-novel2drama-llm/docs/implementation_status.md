...
- `agent/run_store.py`：Agent run store 抽象、`FileRunStore`、`build_run_store()`、artifact index 接口与 `list_runs(...)`，支持 file/sqlite 选择和轻量 run query。
- `agent/sqlite_run_store.py`：本地 SQLite Agent run store v1，使用标准库 `sqlite3` 持久化 runs、requests、reports、events、cancel markers 和 artifact index，并支持 `list_runs(...)`。
...
- `inference/api_server.py`：统一 Foundation `/v1/*` API server 集成，已接入 auth、audit、rate limit middleware、Agent events JSON/SSE endpoint、Agent lifecycle status/cancel/retry/resume endpoints、`GET /v1/agent/runs`、file/sqlite Agent run store 选择和 `/v1/chat` provider SSE streaming。
...
- `tests/test_api_server_foundation.py`：Foundation API server、provider streaming、Agent lifecycle API、Agent runs API 与 SQLite run store 选择测试。
- `tests/test_auth_service.py`：Auth 服务与 Agent lifecycle/run listing scope 测试。
- `docs/p1_agent_run_store.md`：Agent run store 抽象、SQLite store、file/sqlite 选择和 run listing/query 说明。
- `docs/p1_agent_runtime_run_store_writes.md`：Agent runtime run store writes 说明。
- `docs/p1_api_server_integration.md`：API server 集成、run store 选择、Agent runs API 与 Agent lifecycle API 说明。
...
1. P1：继续实现 DB-backed Agent events、Postgres run store、provider-native bidirectional continuation adapter、distributed quota backend、secret-gated provider smoke profiles。
2. P2：接入多 provider、多模态生成/理解、MCP SDK 兼容层、评测、观测、审计、部署。
3. P3：加强 AI 短剧/漫剧专项模型能力，包括故事理解、集数规划、角色一致性、分镜规划、视觉提示词生成、短剧质检评测。

## 禁止误判

- 不能把“Agent run listing/query 完成”说成“DB-backed events、Postgres run store、全文搜索系统或分布式任务队列完成”。
- 不能把“Agent runtime run store writes 完成”说成“DB-backed events、Postgres run store 或分布式任务队列完成”。
