...
runtime_services_completed = false
...
P0 已经把核心研究结果转成第一版工程契约；下一步必须进入 P1 核心服务实现。
...
- 后续必须由 `model_router.py` 使用能力注册表和实例注册表进行真实路由。

## 后续开发顺序

1. P1：实现 router、token/cost、usage ledger、memory、rules、agent runtime、provider adapter base。
