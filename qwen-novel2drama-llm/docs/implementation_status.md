...
P0/P1 已经把核心研究结果转成第一版工程契约和一批可运行服务；OpenAPI 已同步当前 FastAPI runtime，API server 已接入轻量 API key/workspace scope、auth audit、文件状态限流、Agent JSON/SSE 事件接口和 provider SSE streaming，本地模型已接入统一 provider factory，并具备进程内缓存、并发保护和流式输出入口，OpenAI-compatible provider 已支持原生远程 SSE 文本流解析和 tool-call 分片重组，Agent 已能把 provider stream 完成后的 tool calls 桥接到同步 model tool loop，并能在 streamed partial tool call 的 name 与 JSON arguments 完整后提前执行 skill；provider usage reconciliation 已能对比路由预估 usage/cost 与 provider 实际 usage/cost，并把对账结果写回 Agent artifact 与 usage ledger。下一步应继续实现 CI contract check 和 workspace-level budget/quota。

## 当前最重要的未完成项

### P1：底座核心服务剩余项

1. 把 OpenAPI 合同检查加入 CI。
2. 增加 workspace-level quota/budget 与分布式限流 backend。
3. 增加 Agent resume/cancel/retry。
4. 增加 same-stream tool-result injection 或 provider-supported bidirectional tool continuation。
5. 增加 multi-round model tool-loop usage aggregation。
...
1. P1：继续实现 CI contract check、workspace-level budget/quota、Agent resume/cancel/retry、multi-round model tool-loop usage aggregation。
...
- 不能把“P1 router/token/cost/ledger/memory/rules/auth/audit/rate limit/agent runtime/provider adapter/skills/MCP adapter/API server/Agent provider execution/Agent skill loop/local provider/model tool loop/OpenAPI check/streaming/reconciliation 完成”说成“所有核心服务完成”。
