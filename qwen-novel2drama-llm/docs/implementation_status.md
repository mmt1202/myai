...
P1_router_token_cost_usage_memory_rules_agent_runtime_provider_adapter_skills_implemented_v1 = true
...
P0 已经把核心研究结果转成第一版工程契约；P1 已经开始实现 router、token、cost、usage ledger、memory store、rule engine、agent runtime、provider adapter 和 skills registry。下一步应继续实现 MCP adapter 和 API server 集成。
...
1. `mcp/adapter.py`
2. 把 `inference/api_server.py` 接入新的 router/token/cost/memory/rules/agent/provider/skills/envelope。
3. 将 `agent/runtime.py` 接入 provider factory，执行真实 provider call。
4. 将 Skills registry 接入 Agent tool loop。
...
1. P1：继续实现 MCP adapter、API server route integration、Agent provider execution、Agent skill loop。
...
- 不能把“P1 router/token/cost/ledger/memory/rules/agent runtime/provider adapter/skills 完成”说成“所有核心服务完成”。
- 不能把“provider adapter base 完成”说成“所有模型供应商都已经接入”。
- 不能把“Skills registry 完成”说成“完整插件市场或完整 MCP runtime 完成”。
