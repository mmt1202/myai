...
- `inference/api_server.py`：统一 Foundation `/v1/*` API server 集成，并已接入 auth、audit、rate limit middleware。
...
P0/P1 已经把核心研究结果转成第一版工程契约和一批可运行服务；OpenAPI 已同步当前 FastAPI runtime，API server 已接入轻量 API key/workspace scope、auth audit 和文件状态限流，本地模型已接入统一 provider factory，并具备进程内缓存与并发保护，Agent 已具备同步 model-decided tool loop v1 和 JSONL 文件事件流。下一步应继续实现 SSE live events、本地 provider 流式输出、CI contract check 和 workspace-level budget/quota。

## 当前最重要的未完成项

### P1：底座核心服务剩余项

1. 增加 SSE live Agent event endpoint。
2. 增加本地 provider 流式输出入口。
3. 把 OpenAPI 合同检查加入 CI。
4. 增加 workspace-level quota/budget 与分布式限流 backend。
...
1. P1：继续实现 SSE live Agent events、本地 provider 流式、CI contract check、workspace-level budget/quota。
...
- 不能把“Agent event stream 完成”说成“SSE/WebSocket 实时推送已经完成”。
