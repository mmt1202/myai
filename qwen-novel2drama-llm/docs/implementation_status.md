...
AI 短剧/漫剧已经进入 P3 专项能力 v1：小说解析、短剧改编、角色系统、分镜、AI 视频提示词、质量检查和生产 workflow API 已完成仓库级闭环。
...
- `drama/outline.py`：小说转短剧大纲、剧集结构、单集剧情、爆点/钩子/集尾悬念。
- `drama/characters.py`：角色卡、人物关系、角色一致性规则、角色定妆提示词。
- `drama/storyboard.py`：分镜规划、镜头列表、景别、动作、台词、旁白、场景调度。
- `drama/video_prompts.py`：即梦、可灵、Runway、Pika 平台视频提示词适配。
- `drama/quality.py`：剧情连续性、角色覆盖、镜头可拍性、提示词完整性质量检查。
- `drama/pipeline.py`：小说到短剧生产资产包的完整 pipeline 和 artifact 写出。
- `drama/api.py` 与 `drama/fastapi_router.py`：短剧生产 workflow API handlers 和 FastAPI router contract。

### 测试与文档

- Run store、SQLite run store、Postgres run store contract/migration runner/history、worker dispatcher/pool、provider catalog/resilience、multimodal router、MCP SDK compat、eval runner、tracing、audit query、deploy profile、drama pipeline/API、provider continuation/session adapters、provider smoke config、Agent events、Agent runtime、Agent lifecycle、API server、auth、audit/rate limit、workspace quota、quota store、Postgres quota、hardening helpers、metrics、secret resolver、production preflight、skills、MCP 等测试已补充。
...
8. 真实图片/音频/视频生成 provider 的端到端调用与资产托管。

## 后续开发顺序

1. Cloud deployment specialization：Kubernetes/Terraform/managed secrets/managed backups/WAF/CDN/SLO。
2. External integrations：外部 MQ、跨区域调度、真实 billing invoice import/export。
3. AI 资产生产集成：真实图片/音频/视频 provider、资产存储、审核流、人工复核工作台。

## 禁止误判

- P3 drama pipeline 完成，不等于已经接入真实视频生成平台并生成成片。
- P3 quality checks 完成，不等于平台审核或法律合规审查已经完成。
- P3 prompt generation 完成，不等于角色一致性已经由真实视频模型稳定保证。
- P3 workflow API handlers 完成，不等于主 FastAPI 服务已经在生产环境托管所有 drama routes。
