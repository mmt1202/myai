...
def run_smoke(*, project_root: Path = PROJECT_ROOT, model_id: str = "external.openai.primary", live: bool = False, base_url: str | None = None) -> dict[str, Any]:
    ...
    if not live:
        result = provider.generate({**request, "dry_run": True})
        return {"status": "passed", "mode": "dry_run", "env": env, "result": result}
    if not env["api_key_configured"]:
        return {"status": "skipped", "mode": "live", "reason": "api_key_missing", "env": env}
    try:
        result = provider.generate(request)
    except ProviderError as exc:
        return {"status": "failed", "mode": "live", "error": exc.to_error(), "env": env}
    return {"status": "passed" if result.get("status") == "ok" else "failed", "mode": "live", "env": env, "result": result}
...
def main() -> int:
    ...
    report = run_smoke(project_root=Path(args.project_root), model_id=args.model_id, live=args.live, base_url=args.base_url)
