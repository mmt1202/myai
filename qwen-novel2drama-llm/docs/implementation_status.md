...
- `openapi/foundation_api.openapi.yaml`：Foundation API OpenAPI 规格，已同步当前运行时 `/v1/*` endpoints、API key security scheme、ProviderStreamEvent、ProviderToolCall、Agent event schema 和 model tool loop 字段。
...
P0/P1 已经把核心研究结果转成第一版工程契约和一批可运行服务；OpenAPI 已同步当前 FastAPI runtime，API server 已接入轻量 API key/workspace scope、auth audit、文件状态限流、Agent JSON/SSE 事件接口和 provider SSE streaming，本地模型已接入统一 provider factory，并具备进程内缓存、并发保护和流式输出入口，OpenAI-compatible provider 已支持原生远程 SSE 文本流解析和 tool-call 分片重组，Agent 已具备同步 model-decided tool loop v1。下一步应继续实现 provider stream tool-call 到 Agent tool-loop 的桥接、CI contract check 和 workspace-level budget/quota。

## 当前最重要的未完成项

### P1：底座核心服务剩余项

1. 增加 provider stream tool-call 到 Agent tool-loop 的桥接。
...
1. P1：继续实现 provider stream tool-call 到 Agent tool-loop 的桥接、CI contract check、workspace-level budget/quota、Agent resume/cancel/retry。
