...
- `openapi/foundation_api.openapi.yaml`：Foundation API OpenAPI 规格，已同步当前运行时 `/v1/*` endpoints。
...
P0/P1 已经把核心研究结果转成第一版工程契约和一批可运行服务；OpenAPI 已同步当前 FastAPI runtime。下一步应继续实现 auth/API key/workspace scope、本地 provider adapter 和 model-decided multi-turn tool loop。

## 当前最重要的未完成项

### P1：底座核心服务剩余项

1. 增加 auth/API key/workspace scope。
2. 增加本地 provider adapter，把已有本地模型运行时接入 provider factory。
3. 将 request-driven skill loop 升级为 model-decided multi-turn tool loop。
4. 增加 OpenAPI lint/check tooling。
...
- `openapi/foundation_api.openapi.yaml` 已同步当前运行时 endpoints 与 Agent provider/skill loop 字段。

## 后续开发顺序

1. P1：继续实现 auth/API key scopes、本地 provider adapter、model-decided tool loop、OpenAPI lint/check tooling。
...
- 不能把“P1 router/token/cost/ledger/memory/rules/agent runtime/provider adapter/skills/MCP adapter/API server/Agent provider execution/Agent skill loop 完成”说成“所有核心服务完成”。
