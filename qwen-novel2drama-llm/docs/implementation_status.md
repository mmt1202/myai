...
- `agent/run_store.py`：已支持 cooperative worker lease contract。
- `agent/sqlite_run_store.py`：已支持 `run_leases` 表与 SQLite 事务化 claim/renew/release。
- `agent/lifecycle.py`：已支持 worker lease CLI helpers。
...
- Worker lease 已覆盖 file store、SQLite store 和 lifecycle helper 测试。
...
1. P1：Postgres run store、provider-native bidirectional continuation、secret-gated provider smoke profiles、Postgres/distributed quota backend、worker queue/dispatcher。
